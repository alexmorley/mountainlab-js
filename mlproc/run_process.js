...
function LariJob() {
  ...
  this.runProcess = function(
    ...
    for (let okey in m_outputs) {
      // let fname = m_outputs[okey]; // AM: this line isn't used?
      outputs2[okey] = true;
    }
...
function cmd_run_process_lari(processor_name, spec0, opts, callback) {
  ...
  /* let LC =*/ new LariClient(); // AM: What does LC do?
  let p_opts = {};
...
function filter_exe_command(
  ...
  let arguments = [];
  let argfile_lines = [];
  // let console_out_file = ''; AM: Not Used
  for (let key in iop) {
    let val = iop[key];
    if (val !== undefined) {
